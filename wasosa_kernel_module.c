...
#define DEVICE_NAME "wasosa_kernel_module"
#define EXAMPLE_MSG "0123456789ABCDEF\n"
#define MSG_BUFFER_LEN 18
...
static int __init wasosa_kernel_module_init(void)
{
  ...
  major_num = register_chrdev(0, "wasosa_kernel_module", &file_ops);
  if (major_num < 0)
  {
    printk(KERN_ALERT "Could not register device: %d\n", major_num);
    return major_num;
  } else {
    printk(KERN_INFO "wasosa_kernel_module loaded with device major number %d\n",
	   major_num);
    return 0;
  }
}
